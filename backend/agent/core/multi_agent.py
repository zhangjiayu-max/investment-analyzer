...
## 📈 估值数据时效性（主动调用在线查询）
1. 先用 query_valuation 查库内数据（雷牛牛7天→螺丝钉30天→过期数据）
2. **若库内数据超过 7 天未更新（days_old > 7）或标记 is_expired**，主动调用 query_online_valuation 查 akshare 在线最新估值
3. 在线数据返回后，在分析中标注"数据来源：akshare在线（日期）"，与库内数据对比说明差异
4. 若用户明确要求"查最新"，直接调用 query_online_valuation
5. 禁止：库内数据明显过时却不查在线，导致估值判断滞后
