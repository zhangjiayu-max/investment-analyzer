...
from agent.core.multi_agent import run_specialist, run_specialist_with_context, run_cross_review_opinion, _build_portfolio_summary
from agent.infra.blackboard import Blackboard, extract_entry_from_result, reset_global_blackboard
...
from agent.safety.validator import LightValidator
...
def _build_portfolio_summary(prebuilt_context: str = "") -> str:
    ...
    from agent.core.multi_agent import _build_portfolio_summary as _build_ps_db
    return _build_ps_db()
...
def _build_final_synthesis_prompt(specialist_results: list, routed_specialists: list, blackboard=None,
    ...
    # M5：综合报告深度保留 — 5段结构保留推理链条，避免压缩过度
    try:
        from db.config import get_config_bool as _gcb
        deep_synthesis = _gcb("agent.deep_synthesis_enabled", True)
    except Exception:
        deep_synthesis = True
    ...
    if deep_synthesis:
        prompt = (
            ...
            "以用户的私人投资顾问视角呈现。\n\n"
            "## 回复结构要求（M5 深度保留版 — 禁止过度压缩）\n"
            "必须按以下 5 段结构组织，每段缺一不可：\n\n"
            "### 第 1 段：核心结论\n"
            "- 1-2 句话直接回答用户问题，不要绕弯\n"
            ...
            "- 给出总体风险判断（高/中/低）\n\n"
            "### 第 6 段：穿透数据来源标注（如有）\n"
            "- 若分析中使用了基金重仓股/债券持仓数据，必须明确标注每只基金的数据来源：\n"
            "  * akshare 来源：「XX基金 重仓股数据来源：akshare（截至 YYYY-MM-DD）」\n"
            "  * ttfund 兜底：「XX基金 重仓股数据来源：ttfund（akshare 主源失效，已兜底）」\n"
            "  * 本地快照：「XX基金 重仓股数据来源：本地快照（YYYY-MM-DD，可能过期，置信度降低）」\n"
            "  * 部分失效：「XX基金 重仓股数据来源：akshare 部分失效（数据不完整，建议人工核实）」\n"
            "- 上下文中已为每只基金标注了「数据来源: XXX」行，请基于该信息如实转写，禁止编造未标注的来源\n"
            "- 若本次分析未涉及基金穿透查询，本段可省略\n\n"
            "## 通用要求\n"
            "- 使用「我的持仓」「我的方案」「我该怎么做」的用户视角\n"
            "- 每条建议说明「为什么对你的持仓有意义」，基于成本、盈亏、占比\n"
            "- 涉及估值/行情数据时必须说明数据日期，超过7天需提醒「该数据基于约X日前快照，可能滞后」\n"
            "- 数据表格必须标注数据来源和日期\n"
        ...
        prompt = (
            ...
            "- 结尾给出总体风险判断（高/中/低）\n\n"
            "### 5.5 穿透数据来源标注（如有）\n"
            "- 若分析中使用了基金重仓股/债券持仓数据，必须明确标注每只基金的数据来源（akshare / ttfund 兜底 / 本地快照 / 部分失效）\n"
            "- 上下文中已为每只基金标注了「数据来源: XXX」行，请基于该信息如实转写\n"
            "- 若本次分析未涉及基金穿透查询，本节可省略\n\n"
            "### 6. 格式要求\n"
...
def _validate_and_repair(query: str, answer: str, specialist_results: list,
    ...
    try:
        from agent.safety.prompt_defense import validate_financial_data, auto_capture_bad_case
...
def orchestrate(query: str, history: list, rag_context: str = "", cancel_event: threading.Event | None = None, target_specialists: list[str] = None, conversation_id: int = 0, message_id: int = 0, trace_id: str = "") -> dict:
    ...
    try:
        from agent.query.query_rewriter import rewrite_query
    ...
    if _plan_execute_enabled:
        try:
            from agent.core.plan_executor import generate_plan, execute_plan
            available = [
...
def _stream_final_synthesis(query: str, refined_query: str, specialists: list,
    ...
    try:
        if conversation_id > 0 and message_id > 0:
            from agent.infra.orchestrator_optimizer import log_performance_metrics
...
def orchestrate_stream(query: str, history: list, rag_context: str = "", cancel_event: threading.Event | None = None, resume_from: dict | None = None, conversation_id: int = 0, message_id: int = 0, trace_id: str = "", target_specialists: list[str] = None):
    ...
    try:
        from agent.core.pipeline import is_pipeline_enabled, should_use_pipeline, run_pipeline
    ...
    for turn in range(MAX_TURNS):
        ...
        if len(tool_tasks) == 1:
            ...
            # M3：最后位专家注入"魔鬼代言人"角色，强制对前序结论提出质疑
            _bb_written_during_execution = True
            try:
                from db.config import get_config_bool as _gcb
                _devil_inject = _gcb("agent.force_devil_advocate_enabled", True)
            except Exception:
                _devil_inject = True
            for idx, (tc, args, expert_query, agent_key, agent_info) in enumerate(tool_tasks):
                ...
                # M3：串行最后位专家注入魔鬼代言人角色
                if _devil_inject and idx == len(tool_tasks) - 1 and len(tool_tasks) >= 2:
                    ctx += (
                        "\n\n【魔鬼代言人角色】你是本次分析最后发言的专家，"
                        "请对前面专家的结论提出质疑，至少指出 1 个潜在盲点或风险：\n"
                        "- 是否存在价值陷阱？（低估可能有更深原因）\n"
                        "- 是否低估了下行风险？（黑天鹅/尾部风险）\n"
                        "- 推理链是否有逻辑跳跃？（因果倒置/相关≠因果）\n"
                        "- 是否遗漏了重要前置假设？\n"
                        "即使方向一致也要找盲点，但质疑必须有数据或逻辑支撑，不得为反驳而反驳。"
                    )
...
def _schedule_tool_eval(query: str, specialist_results: list[dict]):
    ...
    try:
        from agent.infra.tool_tracker import evaluate_tool_calls_async
...
async def _do_auto_evaluation(conv_id: int, msg_id: int, result: dict):
    ...
    from agent.eval.conversation_evaluator import ConversationQualityEvaluator
    from agent.eval.conversation_evolution import process_conversation_evaluation
