...
def _parse_arbitration_output(answer: str) -> dict:
    ...
    if json_matches:
        for raw_json in json_matches:
            try:
                ...
                if "condition_framework" in data or "diverggence_analysis" in data:
                    return {
                        "diverggence_analysis": data.get("diverggence_analysis", data.get("divergence_analysis", "")),
                        "condition_framework": data.get("condition_framework", []),
    ...
    # ── Level 2: 正则提取表格行 ──
    # 匹配 | 内容 | 内容 | 置信度 | 格式，跳过分隔符行
    table_row_pattern = r'\|\s*([^|]+?)\s*\|\s*([^|]+?)\s*\|\s*(\d+%?)\s*\|'
    table_rows = re.findall(table_row_pattern, answer)

    condition_framework = []
    for condition, action, confidence in table_rows:
        c = condition.strip()
        a = action.strip()
        # 跳过表头行和分隔符行
        if c in ("你的判断", "条件", "判断") or a in ("对应行动", "行动"):
            continue
        # 跳过分隔符行 (如 --- or ----)
        if re.match(r'^[-]+$', c) or re.match(r'^[-]+$', a):
            continue
        condition_framework.append({
            "condition": c,
            "action": a,
    ...
    for pat in div_patterns:
        ...
        if m:
            divergence_analysis = m.group(1).strip()
            break
    ...
    for pat in tradeoff_patterns:
        m = re.search(pat, answer, re.DOTALL)
        if m:
            key_variables = m.group(1).strip()
            break
    ...
    for pat in rec_patterns:
        m = re.search(pat, answer, re.DOTALL)
        if m:
            final_recommendation = m.group(1).strip()
            break

    has_structured = bool(condition_framework) or bool(divergence_analysis)
    ...
    return {
        "diverggence_analysis": divergence_analysis,
        "condition_framework": condition_framework,
...
{experts_text}"""
    ...
    # 解析条件判断框架
    parsed = _parse_arbitration_output(answer)
    ...
    return {
        ...
        "condition_framework": parsed.get("condition_framework", []),
        "diverggence_analysis": parsed.get("diverggence_analysis", ""),
