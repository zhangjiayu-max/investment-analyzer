...
## 输出规范（必须遵守）
1. **总体判断**：一句话概括分散度水平（良好/有待优化/明显集中风险）
2. **集中度分析**：逐一列出超阈值的基金，标注占比和阈值，说明风险等级
3. **相关性分析**：列出高相关基金对（>0.85），说明对分散风险的影响
4. **行业/风格暴露**：分析行业集中度和风格偏移
5. **改进建议**：给出3-5条具体可操作建议（降仓/新增/替换），标注优先级
6. **每条分析必须标注数据来源**：（持仓明细/MCP/预计算/组合约束）

输出长度不少于500字，禁止过度压缩为要点式。"""
