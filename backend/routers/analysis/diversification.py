...
1. **总体判断**：一句话概括分散度水平（良好/有待优化/明显集中风险），给出分散度评分（0-100）
2. **集中度分析**：逐一列出超阈值的基金，标注占比和阈值，说明风险等级（高/中/低）
3. **相关性分析**：列出高相关基金对（>0.85），说明对分散风险的影响及建议处理方式
4. **行业/风格暴露**：分析行业集中度和风格偏移，引用具体占比数据
5. **改进建议**：给出3-5条具体可操作建议（降仓/新增/替换），标注优先级和调整比例
6. **每条分析必须标注数据来源**：（持仓明细/MCP/预计算/组合约束）

## 输出格式要求
- 每个章节必须包含完整段落和分析过程，禁止仅用要点罗列
- 引用具体数值（占比%、相关系数、阈值等）支撑结论
- 输出长度严格控制在500-800字，禁止过度压缩为要点式
- 末尾给出"分散度综合评分"和"主要风险点"小结"""
