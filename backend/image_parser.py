...
from config import get_vision_config, IMAGE_PARSER_MODEL_TYPE
...
重要提取规则：
1. **市场温度**：图片顶部或标题附近通常有一个"市场温度"或"温度"数值（如25°、35.5等），必须提取。如果确实找不到，设为null
2. **更新日期**：从图片标题或顶部提取日期，格式统一为"YYYY-MM-DD"（如"2026-05-28"）。如果图片显示"5月28日"，转换为"2026-05-28"
3. 如果图片中没有某项数据，对应字段设为null
4. 估值状态根据百分位判断：<30%低估，30-70%适中，>70%高估
5. 背景颜色请根据指数行的实际背景颜色填写：绿色/浅绿色表示低估，黄色/橙色表示适中，红色/粉红色表示高估
6. 只输出JSON，不要其他文字"""
...
class ImageParser:
    ...
    def __init__(self, model_type: str = ""):
        ...
        参数:
            model_type: 模型类型标签，默认从 config.IMAGE_PARSER_MODEL_TYPE 读取
        """
...
class DDImageParser:
    ...
    def _normalize(self, data: dict) -> dict:
        ...
        # 统一日期格式为 YYYY-MM-DD
        raw_date = data.get("更新日期") or ""
        normalized_date = self._normalize_date(raw_date)

        # 提取市场温度（可能是数字或带°的字符串）
        raw_temp = data.get("市场温度")
        temperature = self._parse_temperature(raw_temp)

        return {
            "ok": True,
            "update_date": normalized_date,
    ...
    def _parse_temperature(raw_temp) -> float | None:
        """提取市场温度数值。支持 "25"、"25.5"、"25°"、"25.5°" 等格式。"""
        if raw_temp is None:
            return None
        import re
