...
function getOption(echarts) {
  ...
  return {
    ...
    title: props.title ? {
      ...
      left: 'center',
      top: 8,
      textStyle: { color: theme.value.titleColor, fontSize: 13, fontWeight: 600, fontFamily: theme.value.fontSerif },
    } : undefined,
    tooltip: {
      backgroundColor: isDark.value ? 'rgba(13,18,32,0.95)' : '#ffffff',
      borderColor: theme.value.borderColor,
      borderWidth: 1,
      textStyle: { color: theme.value.titleColor, fontSize: 12, fontFamily: theme.value.fontMono },
      trigger: 'item',
    ...
    series: [{
      ...
      emphasis: {
        label: { show: true, fontSize: 13, fontWeight: 'bold', fontFamily: theme.value.fontMono },
